...
class BioProc:
    def __init__(self, params, model_mode, parameter_values=def_parameter_values, avg_dev=30, plot_fitness=False,
                 plot_devs=True, load_init=True):
        ...
        snId = 1.0 / (1 + pow(abs((np.linspace(0, 1, s_width) - c) / a),
                              2 * b)) * 2 * self.amp  # fitness bell shaped instructions #s_width*[self.amp*2]
        snFF = 1.0 / (1 + pow(abs((np.linspace(0, 1, self.multiplier * s_width) - c) / a),
                              2 * b * self.multiplier)) * 2 * self.amp  # self.multiplier*s_width*[self.amp*2]
        ...
        for i in range(self.multiplier):
            ...
            idFF[
            (i + 1 + self.multiplier * 2) * s_width:(i + 1 + self.multiplier + self.multiplier * 2) * s_width] = snFF
